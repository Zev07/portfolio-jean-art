...
export default function ObrasPage() {
    ...
    const fetchWorks = async () => {
      try {
        ...
        const formattedWorks: WorkItem[] = data.map((item: any) => ({
          ...
          imageUrl: item.mainImage ? urlFor(item.mainImage).width(800).url() : '/images/placeholder.png'
        }));
  ...
  return (
    <main className="min-h-screen pb-20 bg-background overflow-x-hidden selection:bg-[#D91C2B] selection:text-white transition-colors duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {sketches.map((work) => (
                <div
                  key={work.id}
                  onClick={() => handleOpenImage(work)}
                  className="group cursor-pointer relative border-2 border-black dark:border-white bg-white p-1 hover:rotate-2 hover:scale-105 transition-all duration-200 shadow-[4px_4px_0px_#ccc] hover:shadow-[6px_6px_0px_#D91C2B]"
                >
                  <div className="overflow-hidden bg-zinc-200">
                    <WorkCard work={work} />
                  </div>
                  <div className="absolute -top-2 -right-2 bg-[#D91C2B] text-white text-[10px] font-bold px-2 py-0.5 transform rotate-12 opacity-0 group-hover:opacity-100 transition-opacity">
                      CONFIDENTIAL
                  </div>
                </div>
              ))}
      ...
      <ImageModal
        isOpen={isModalOpen}
        imageUrl={selectedWork?.imageUrl || null}
